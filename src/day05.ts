...
function main() {
  ...
  const input = readInput("05", runTypeArg === "full");
  ...
  for (const ingredient of input.ingredients) {
    ...
    for (const range of input.freshRanges) {
      if (ingredient >= range.low && ingredient <= range.high) {
        // console.log(
        //   "ingredient is good because it falls in range #",
        //   idx,
        //   range
        // );
        freshCount++;
...
function solve2(input: ParsedInput): void {
  ...
  input.freshRanges.sort((a, b) => a.low - b.low);

  //merge ranges
  // console.log(input.freshRanges);
  ...
  for (const range of input.freshRanges) {
    ...
    if (currentRange.high < range.low) {
      freshCount += currentRange.high - currentRange.low + 1;
      currentRange = range;
    } else if (currentRange.high < range.high) {
